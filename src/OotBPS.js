...
class BPS {
    ...
    handleBps(rom, patch) {
        try {
            ...
            var basename = path.basename(patch.name)
            var ext = '.' + rom.name.split(".").pop();
            fs.writeFileSync("./temp/" + basename+ext, ret);
            return "./temp/" + basename+ext;
        } catch (e) {
            throw e;
        }
    }
    ...
    tryPatch(rom, bps) {
        var bpsdata;
        var romdata;
        var rom_hold = fs.readFileSync(rom);
        romdata = { bytes: rom_hold.buffer, name: rom, mime: "bps" };
